...
# . means the current same directory
...
def initialize_exp(params):
    ...
    # print(type(params)) <class 'argparse.Namespace'>

    # create a logger
    # todo learn the logger here
...
def concat_batches(x1, len1, lang1_id, x2, len2, lang2_id, pad_idx, eos_idx, reset_positions):
    ...
    # here the len1, len2 contains already 2 </s> inside
    lengths = len1 + len2
    ...
    # x is the new tensor. x1 is the given sentences tensor
    ...
    for i in range(bs):
        ...
        if reset_positions:
            # reset the position for sent2 (restart from 0), subtract len1[i] for each of sent2's positions
