...
class SentenceEmbedder(object):
    ...
    def reload(path, params):
        ...
        # the last two parameters: is_encoder, with_output
        model = TransformerModel(pretrain_params, dico, True, True)
        # class TransformerModel(nn.Module):
        # load_state_dict: TransformerModel class inherits torch.nn.Module
    ...
    def get_embeddings(self, x, lengths, positions=None, langs=None):
        ...
        Outputs:
            `sent_emb` : FloatTensor of shape (bs, out_dim)  the hidden representation in the last layer of the first </s>
        With out_dim == emb_dim
        """
        slen, bs = x.size()
        assert lengths.size(0) == bs and lengths.max().item() == slen
        # get transformer last hidden layer
        # call the corresponding forward method
        tensor = self.model('fwd', x=x, lengths=lengths, positions=positions, langs=langs, causal=False)
        # slen: the longest sequence length
        assert tensor.size() == (slen, bs, self.out_dim)
        # tensor[0] is a tensor of shape (batch_size, model_dimension) that corresponds to
        # the first hidden state of the last layer for each sentence.
        return tensor[0]
